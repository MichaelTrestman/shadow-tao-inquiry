...
In bittensor, TAO can exist in two states for a given coldkey address:

- **Free balance** — liquid TAO held directly in the account, transferable immediately
- **Staked balance** — TAO committed to one or more hotkeys across one or more subnets, earning emissions but not immediately liquid
...
In bittensor, the relationship between coldkeys and hotkeys determines network role:

- **Validator**: the coldkey owns a hotkey that holds `validator_permit = True` on at least one subnet. Validators are elected to set weights and earn dividends based on stake weight.
- **Miner**: the coldkey owns a hotkey registered on a subnet without validator permit. Miners serve inference or other subnet-specific tasks.
- **Subnet owner**: the coldkey is `owner_ss58` on one or more subnets. Subnet owners set the rules and earn a share of subnet emissions.
- **Delegator**: the coldkey has staked TAO to hotkeys it does not own — nominating a validator to earn emissions on its behalf.
